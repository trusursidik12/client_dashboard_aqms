...
class OutdoorVtwo extends ResourceController
{
        ...
        try {
            if (!empty($stationID)) {
                ...
                $response['data'] = [
                    ...
                    'date_time' => date('d F Y', strtotime($ispu->date_time)) . ' jam 15:00',
                    'pm10' => $ispu->pm10,
                    'pm25' => $ispu->pm25,
                    'so2' => $ispu->so2,
                    'co' => $ispu->co,
                    ...
                    'c_no2' => 1,
                    'c_hc' => 1,
                    'run1' => "Stasiun {$ispu->nama_stasiun} {$ispu->date_time} PM2.5:{$data->pm25} SO2:{$data->so2} CO:{$data->co} O3:{$data->o3} NO2:{$data->no2} VOC:{$data->voc} SUHU:{$data->temperature} RH: % TEK: {$data->pressure} mBar WS: {$data->ws} m/s WD: {$data->wd}",
                    'run2' => "PM2.5:" . $pm25 . " SO2:" . $so2 . " CO:" . $co . " O3:" . $o3 . " NO2:" . $no2,
                ];
